...
public class LaporanKasir extends javax.swing.JPanel {
    ...
    private int getTotalData(){
        ...
        try {
            ...
            Logger.getLogger(LaporanKasir.class.getName()).log(Level.SEVERE,null,e);
        }
    ...
    private void showPanel(){
        panelMain.removeAll();
        panelMain.add(new LaporanKasir());
        panelMain.repaint();
...
private void searchData() {
    ...
    try {
        // Query pencarian berdasarkan Nama Kasir (Nama_User)
        String sql = "SELECT usr.Nama_User AS Kasir, SUM(det.Jumlah) AS Total_Jumlah " +
                     "FROM detail_transaksi det " +
                     "INNER JOIN transaksi tr ON tr.ID_Transaksi = det.ID_Transaksi " +
                     "INNER JOIN user usr ON usr.ID_User = tr.ID_User " +
                     "WHERE usr.Nama_User LIKE ? " +
                     "GROUP BY usr.Nama_User " +  // Mengelompokkan berdasarkan nama kasir
                     "ORDER BY usr.Nama_User";
        ...
        try (PreparedStatement st = conn.prepareStatement(sql)){
            ...
            // Menambahkan data yang ditemukan ke model
            while (rs.next()) {
                String kasir = rs.getString("Kasir");
                int totalJumlah = rs.getInt("Total_Jumlah");
